...
class _DailyInfoPagesState extends State<DailyInfoPages> with AutomaticKeepAliveClientMixin<DailyInfoPages> {
      ...
      body: SafeArea(
        child: BlocBuilder<SearchBloc, SearchState>(
          builder: (context, state) {
            if (state.isInitial) {
              return CenteredMessage(message: 'Starting search...', iconData: Icons.timelapse);
            }
            if (state.isLoading) {
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Container(
                ...
                child: Text(
                  searchResult.title ?? "Title not present",
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 17.0,
                    color: Colors.black,
  ...
  Future _selectDate(BuildContext context) async {
    DateTime picked = await showDatePicker(
        context: context, initialDate: DateTime.now(), firstDate: DateTime(1995, 6, 17), lastDate: DateTime.now());
