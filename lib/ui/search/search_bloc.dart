...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  SearchBloc(this._dailyInfoRepository) : super(SearchState.initial(DateTime.now()));

  void onSwipeSearchInitiated() {
    add(DateInitialSearchEvent((b) => b..date = DateTime.now()));
  }

  void onDatePickSearchInitiated(DateTime pickedDate) {
    add(DateChosenSearchEvent((b) => b.date = pickedDate));
  }

  void fetchNextSwipePage(DateTime nextDate) {
    add(DateNextSearchEvent((b) => b..date = nextDate));
  }
  ...
  Stream<SearchState> mapInitialDailySearch(DateInitialSearchEvent event) async* {
    if (event.date == null) {
      ...
      if (dateIsValid(event.date)) {
        ...
        yield SearchState.failure('Date is not valid!');
    }
  }
